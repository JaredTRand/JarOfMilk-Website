...
unzip = lambda path, member, encoding="U8": uncompress(
    zipfile.ZipFile(path), member, encoding
)

untargz = lambda path, member, encoding="U8": uncompress(
    tarfile.open(path, "r:gz"), member, encoding
)
    ...
    while wordlist:
        ...
        while new_word_len == word_len:
            ...
            if not reconv_table.get(new_word) and (
                test_word != word
                or (
                    tomanyptn.search(word) and word != translate(new_word, reconv_table)
                )
            ):
...
def main():
    ...
    t_wordlist = set()
    s_wordlist = set()
    t_wordlist.update(ezbigParser(tbe_dest), tsiParser(lbt_dest))
    s_wordlist.update(wubiParser(tbe_dest), zrmParser(tbe_dest), phraseParser(pyn_dest))
